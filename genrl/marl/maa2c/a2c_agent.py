...
class A2CAgent:
    ...
    def setup_model(self,load_model):
        ...
        if load_model is not None:
            self.actorcritic.load_state_dict(torch.load(model_path,map_location=torch.device(self.device)))
    ...
    def collect_rollouts(self,states):
        ...
        self.states = torch.FloatTensor([sars[0] for sars in trajectory]).to(self.device)
        self.next_states = torch.FloatTensor([sars[1] for sars in trajectory]).to(self.device)
        self.actions = torch.LongTensor([sars[2] for sars in trajectory]).to(self.device)
        self.rewards = torch.FloatTensor([sars[3] for sars in trajectory]).to(self.device)
    ...
    def get_traj_loss(self,curr_Q,done):
        discounted_rewards = np.asarray([[torch.sum(torch.FloatTensor([self.gamma**i for i in range(self.rewards[k][j:].size(0))])* self.rewards[k][j:]) for j in range(self.rewards.size(0))] for k in range(self.num_agents)])
        discounted_rewards = np.transpose(discounted_rewards)
        value_targets = self.rewards + torch.FloatTensor(discounted_rewards).to(self.device)
        ...
        self.entropy = -torch.mean(torch.sum(dists * torch.log(torch.clamp(dists, 1e-10,1.0)), dim=-1))
    ...
    def update_policy(self):
        ...
        self.grad_norm = torch.nn.utils.clip_grad_norm_(self.actorcritic.parameters(),0.5)
        self.optimizer.step()
